...
//        55.650937, 37.418443
        ...
        request.getPhotoMessage().getCamMessage().setLatitude(55.650937 + (Math.random() * 0.002 - 0.001));
        request.getPhotoMessage().getCamMessage().setLongitude(37.418443 +  (Math.random() * 0.002 - 0.001)); // Долгота 37-47
        ...
        for (int i = 1; i <= 3; i++) {
            ...
            construction.setLatitude(request.getPhotoMessage().getCamMessage().getLatitude() + (Math.random() - 0.5) * 0.01); // ±0.005 от камеры
            construction.setLongitude(request.getPhotoMessage().getCamMessage().getLongitude() + (Math.random() - 0.5) * 0.01); // ±0.005 от камеры
